...
for(var o,s,u=h["cp"+i],c=0,a=Math.max(r.length,n.length);a>c;++c){
...
var c=navigator.userAgent,a=/(iPhone|iPad|iPod|iOS)/i.test(c),d=/Windows\sPhone/i.test(c),p=/(Android)/i.test(c),f=/MicroMessenger\/([\d\.]+)/i.test(c),g=/mac\sos/i.test(c),l=/windows\snt/i.test(c)&&!d,m=/MPAPP\/([\d\.]+)/i.test(c),w=p&&/miniprogram/.test(c.toLowerCase())||"miniprogram"==window.__wxjs_environment,v=f&&/wxwork/i.test(c),h={
"cp-1":function(n,t){
return t>n;
},
cp0:function(n,t){
